...
void moveplayer(int dx, int dy){
    ...
    if (newX>=0 && newX<WIDTH && newY>=0 && newY<HEIGHT && maze[newY][newX]!=WALL){         // check if new pos isnt a wall 
        player->x =newX;
        player->y =newY;
    }
}
...
void drawendscreen(){
    ...
    DrawText("Press Enter to continue", (WIDTH*CELLSIZE/2) - 160, HEIGHT*CELLSIZE/2 +10, 20, WHITE);
}
    ...
    while (!WindowShouldClose()){
        ...
        if (gameover) {
            ...
            DrawText("Press ENTER to continue", 
                    (WIDTH*CELLSIZE/2) - 120,
                    (HEIGHT*CELLSIZE/2) + 50,
                    20, WHITE);

            if (IsKeyPressed(KEY_ENTER)) {
                break;   // exit maze, return result to main
            }
        }
