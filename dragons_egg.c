...
fireballnode* fireballs=NULL;                         // head of LL for fireballs
obstNode* obstacles=NULL;                              // head of LL for obstacles
keynode* keys=NULL;                                     // head of LL for keys


void add_obst(Vector2 pos){     //LL of obstacles
...
void reset_dragons_egg_state() {
    frameCounter=0;
    won=false;
    lost=false;

    player1.pos=(Vector2){0, 0};
    player1.lives=3;

    dragon= (Vector2){GRID_WIDTH-1, GRID_HEIGHT-1};

    // free old lists if any (safety)
    fireballnode* f;
    while (fireballs) { f=fireballs; fireballs=fireballs->next; free(f); }
    obstNode* o;
    while (obstacles) { o=obstacles; obstacles=obstacles->next; free(o); }
    keynode* k;
    while (keys) { k=keys; keys=keys->next; free(k); }
...
int dragons_egg(){
    ...
    while (!WindowShouldClose()){            // drawinggg starts
        ...
        if (won || lost) {
            DrawText("Press ENTER to continue", 200, 360, 20, WHITE);

            if (IsKeyPressed(KEY_ENTER)) {
                break;   // EXIT mini-game cleanly
            }
        }
